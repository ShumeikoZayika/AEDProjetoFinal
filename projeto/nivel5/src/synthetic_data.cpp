...
void loadSyntheticData(Graph& g) {
    ...
    // "The Dukes" is, in reality, another name for APT29 / Cozy Bear.
    // We model it as a SEPARATE node here on purpose: it simulates a second
    // intel feed reporting the same actor under a different name, so the
    // entity-resolution algorithm (Nivel 3) has a real duplicate to detect.
    Node dukes("dukes", "The Dukes", EntityType::THREAT_ACTOR);
    dukes.properties["origin"] = "Russia";
    dukes.properties["alias"]  = "Cozy Bear";
    g.addNode(dukes);
    ...
    // "The Dukes" (= APT29, second feed) — same activity reported under a
    // different name. Entity resolution should later link dukes <-> apt29.
    g.addEdge("dukes", "sunburst",        RelationType::USES,
              "Reported by a second feed attributing SUNBURST to 'The Dukes'");
    g.addEdge("dukes", "camp-solarwinds", RelationType::ATTRIBUTED_TO);
